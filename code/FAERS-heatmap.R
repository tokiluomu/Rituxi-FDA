...
FAERS <- read_excel("FAERS.xlsx")

data_value <- FAERS[,1:5]
data <- FAERS[, c(1, 6:10)]
...
ggplot(data_long, aes(x = variable, y = pt_name_en, fill = value)) +
  ...
  scale_fill_manual(values = c("0" = "#FCBEB0", "1" = "#E73847"), 
...
ggsave('FAERS-heatmap.png', device = "png", dpi = 600,width=13, height=10,units = "in")
