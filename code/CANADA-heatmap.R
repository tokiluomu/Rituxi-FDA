...
data_value <- CANADA[,1:5]
data <- CANADA[, c(1, 6:10)]
...
ggplot(data_long, aes(x = variable, y = pt_name_en, fill = value)) +
  ...
  scale_fill_manual(values = c("0" = "#BEE9F5", "1" = "#457B9D"), 
...
ggsave('CANADA-heatmap.png', device = "png", dpi = 600,width=13, height=10,units = "in")
